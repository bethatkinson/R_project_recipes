...
```{r}
# 1. Create a named vector of available labels 
# The arsenal package has the function "label()"
label(d1$age)

vlabels <- unlist(sapply(d1, FUN=label))

# 2. Create a data frame of of variables with labels [vars=variable name, labels=variable label]
tmp1 <- data.frame(vars=names(vlabels), labels=vlabels)
# 3. Create a data frame of all variables in the dataset
tmp2 <- data.frame(vars=names(d1)) 
# 4. Merge together so all variables are represented
labeldata <- merge(tmp1, tmp2, all=T, by='vars') 

# Check to see what is missing and add in your own labels 
# 5. Check to see which variables do not have labels
labeldata %>% filter(is.na(labels))
# 6. Add labels to those variables
# Note: order must match to order of variables with missing labels in  'labeldata'
labeldata$labels[is.na(labeldata$labels)] <- c('Cardiogenic shock','Current smoker',
                                               'Diabetes mellitus', 'In-hospital death',
                                               'Hypertension','MI complication',
                                               'Moderate/Severe renal disease', 'Recent MI')
...
# For example vlabels$dt.death or vlabels[['dt.death']] will return 'Death date'
vlabels <- as.list(labeldata$labels)
names(vlabels) <- labeldata$vars

# create a vector of covariates
covar <- c('age','gender','recentmi','cardioshock','diabetes','hypertension','bmi','currsmoker',
