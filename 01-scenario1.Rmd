...
In the output below, the `class()` function is returning the class of each variable. Typical R classes for variables in a dataset include: character, numeric, integer, Date, logical, factor. More information on classes and data types can be found in the [New Employee R Training Material](http://bsiweb.mayo.edu/sites/default/files/datatypesandstructures.docx).
...
dfSummary(dat1, plain.ascii=FALSE, style='grid',
          graph.col=FALSE, omit.headings=TRUE)
...
```{r duplicates}
# count how many times each clinic number appears in dat1  
table(table(dat1$clinic))


# This next line of code first identifies which clinic numbers are duplicated: "duplicated(dat1$clinic)"
# Then, it selects those clinic numbers from "dat1$clinic" and returns only one instance of each duplicated clinic number using "unique()".

tmp <- unique(dat1$clinic[duplicated(dat1$clinic)])

# which rows have a duplicate?
dup.rows <- which(dat1$clinic %in% tmp)

# show a portion of the data with the duplicates
kable(dat1[dup.rows, c('clinic','case','age','arm','sex')])
```
...
dat1 %>% filter(clinic %in% tmp) %>%
         select(clinic, case, age, arm, sex) %>%
        kable()
```
...
# now compare after matching by clinic number
tmp2 <- comparedf(x=sas_dat1, y=excel_dat1, by='clinic')
tmp2
...
summary(comparedf(x=sas_dat1, y=excel_dat1, by = "clinic", tol.vars = "."))
```
...
* [dart](http://hsrinf.mayo.edu/activity/s200555/dart/00Index.html)
...
ok <- match(c('clinic','case','age','sex','bmi','birthdt','resintdt'), labeldata$vars)

# 2) for those locations, add in new labels
labeldata$labels[ok] <- c("MCN", "Case", "Age", "Sex", "BMI", "Birthdate", "ResInt Date")
...
We can change the variable types using the dplyr package and mutate_at(). We will not include clinic and case in the tables since they are unique identifiers.  The code shown below makes all of these variables factors instead of numeric variables.

```{r results = 'asis'}
library(dplyr)
cols <- vars("clinic", "case", "fustat", "ps", "mdqualitys")
new_dat <- dat1 %>% mutate_at(cols, funs(factor(.)))
```

Here is another use of tableby to explore the data, removing the variables clinic and case on-the-fly using the `select()` function.

```{r, results='asis'}
summary(tableby(~ ., 
                data=subset(dat1, select = c(-clinic, -case))), 
...
# LHS=Arm, RHS=all other variables except clinic and case 
# exclude RHS identifies variables to leave out from right hand side (as indices)
excludeRHS <- which(names(dat1) %in% c("clinic", "case", "arm"))
