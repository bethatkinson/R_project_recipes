...
```{r message=FALSE}
# Before doing any work, you are strongly encouraged to set this option in each of your programs (default for later versions of R)
...
tab2 <- tableby(arm ~ age + sex + ps + bmi + alkphos + mdqualitys + ageord + birthdt, data=dat2, control=mycontrol)
...
tab2_males <- tableby(arm ~ age + ps + bmi + alkphos + mdqualitys + ageord + birthdt, data=dat2, control=mycontrol, subset=sex=='Male')
...
```{r}
ggplot(dat2, aes(age, bmi, color=arm)) + geom_point(alpha=.2)  + facet_grid(~sex) + 
...
# openxlsx, cannot real .xls - only .xlsx
